...
| **[milestone](skills/milestone/)** | 会話間で永続化する開発トラッカー。各マイルストーンは自己完結型カプセル：目標、ステータス付きサブタスク、決定、コード参照、コンテキストログ。Plan mode および全プランニングスキルと統合。 |
| **[usage-tracker](../skills/usage-tracker/)** | 全ツール呼び出しを `~/.claude/usage.jsonl` に記録する PostToolUse フック。各ユーザーリクエストのコストをプロジェクト・セッション・日・ツール別に正確に把握。 |
...
> **3つの会話でfeatureを完成させた。4つ目はゼロからスタートする。コンテキストが生き残らないから。**

milestone は、将来のどの会話でも開発作業を再開するために必要なすべてを保存します — 目標、ステータス付きサブタスク、アーキテクチャ上の決定、コード参照、そして何をしたか・なぜしたかの逆時系列ログ。名前でマイルストーンをロードし、すぐに作業を開始できます。

### 仕組み

- `/milestone` — すべてのマイルストーンのステータスと進捗を一覧表示
- `/milestone <名前>` — 完全なコンテキストをロード（ファジーマッチ）
- `/milestone init <名前>` — コードベースに基づくサブタスク付きの新しいマイルストーンを作成
- `/milestone add/done/update` — サブタスク、決定、コンテキストを管理

### 主要な設計決定

- **追記専用のコンテキストログ** — 履歴を削除しない、訂正のみ追加
- **プランナーの検出** — インストール済みのプランニングスキルを自動検出
- **グローバルスキル、ローカルデータ** — プロジェクトごとに `.milestones/` を作成
- **8つのNEVERルール** — 些細なマイルストーン禁止、重複禁止、アクティブ最大10
...
> **Claude Max を使っている。トークン課金なし。でもどのプロジェクト、会話、リクエストがコンテキスト制限を消費しているか全くわからない。**

usage-tracker がこれを解決します。PostToolUse フックがすべてのツール呼び出しをトークン数・プロジェクト・それを引き起こしたユーザーリクエストとともにキャプチャし、不透明な使用履歴をリクエスト・プロジェクト・セッション・ツール・日別の実用的な内訳に変えます。

### 仕組み

```
ユーザー: "auth モジュールをレビュー"
  └─ Read auth.module           → 1,200 tok   ┐
  └─ Grep hook                  →    80 tok   │ 同じ"リクエスト"
  └─ Read AuthService.php       → 2,400 tok   │ → 合計: 4,980 tok
  └─ Bash lint auth/            → 1,300 tok   ┘
```

各エントリに保存される情報: タイムスタンプ、セッション、プロジェクト、ツール、モデル、ラベル、リクエストテキスト、トークン数。レポートスクリプトが実際に行動できる内訳に集約します。

### 見えにくい重要な点

フックはツール呼び出しを個別にキャプチャしますが、Claude はすべてのリクエストで会話履歴全体を送信します。これにより**非線形な過小評価**が生じます:
...
絶対コストとしてではなく、プロジェクト・セッション・リクエストタイプを比較する**相対指数**として使用してください。

最大の盲点:
- **Agent 呼び出し** — サブエージェントの会話は完全に不可視（ログ 500 トークン = 実際には 20,000+ の可能性）
- **長い会話** — コンテキストは二乗で蓄積される; 独立したタスクは新しい会話で開始する
- **アクティブなスキル** — ロードされた SKILL.md はリクエストごとに固定のオーバーヘッドを追加

### コマンド

```bash
/usage-tracker install        # フック + スクリプトのセットアップ
/usage-tracker report hoy     # 今日のレポート
/usage-tracker report semana  # 過去 7 日間
/usage-tracker top-requests   # 最もコストの高い 15 件のリクエスト
/usage-tracker status         # フックが有効か確認
```
