...
**Step 3**: Renderizar (A) tabla resumen + (B) desglose por milestone según [`rendering-rules.md`](rendering-rules.md).
...
**Step 6**: Verificar dependencias entre pendientes antes de sugerir siguiente.
...
**Step 3 — Informe**: tabla ✅/🟡/🔴/⚠️ — qué milestones al día, cuáles desactualizados, qué falta. Incluir fila "config.yml" con ✅ válida / 🟡 migrada / 🔴 ausente.
...
3. `/milestone start <slug>` → abre ventana nueva.
4. Nueva sesión arranca con snapshot fresco (~100 tok) y ejecuta.
5. Al terminar: `/milestone done` o `/milestone update`.
...
2. Mostrar solo `/milestone start <slug>` para pegar en nueva ventana del plugin.
3. STOP — no cargar contexto aquí; la ventana nueva lo hará.
...
2. Cargar contexto del snapshot (ya en `MEMORY.md`).
3. Mostrar: objetivo, `[~]` pendientes, `[ ]` siguientes, siguiente acción sugerida.
4. Preguntar: aprobar `[~]` o empezar `[ ]` siguiente.
...
Precondición: `/milestone update <slug>` ANTES.
...
| Entorno | Modo | Acción |
|---------|------|--------|
| Terminal nativo | A | (1) `/milestone update` → (2) `bash ~/.claude/milestone-new-session.sh "$(pwd)" "<slug>"` |
| IDE — sesión origen | A | (1) `/milestone update` → (2) NO script. Mostrar solo `/milestone start <slug>` |
| IDE — sesión nueva | B | NO pre-start sync. Cargar snapshot, mostrar objetivo + pendientes, preguntar acción |
...
Siguientes no iniciadas [ ]:
- X.Y — <nombre>

¿Qué hacemos?
  a) Aprobar las [~] pendientes (/milestone done <slug> X.Y)
  b) Empezar subtarea X.Y
```
...
Edit mínimo: `old_string` solo la línea del checkbox. Si la línea tenía anotación `` `⏳ PR #<n>` `` (R13), retirarla en el mismo Edit. Añadir entrada en `## Contexto`. Actualizar snapshot. Si team mode → `milestone-sync.sh push <root> <slug>`.
